...
class RouteTrie:
    ...
    def insert(self, request_path, handler):
        ...
        for sub_path in sub_path_list[1:]:
            ...
            if sub_path in current_node.sub_path_dict:

                current_node = current_node.sub_path_dict[sub_path]
                ...
                current_node.sub_path_dict[sub_path] = child_route_trie_node

                current_node = child_route_trie_node
    ...
    def __str__(self):
        ...
        def print_node(node, output):
            output[0] += (f"Node handler: {node.handler}\n")

            # print(output_str)

            for sub_path in node.sub_path_dict:
                output[0] += (f"Sub path: {sub_path}\n")

                print_node(node.sub_path_dict[sub_path], output)
...
class Router:
    ...
    def __init__(self, root_handler=None, no_handler=None):
...
print(router.lookup("/"))  # Print None
...
print(router.lookup("/")) # Print None
print(router.lookup("/home")) # Print home
print(router.lookup("/1234")) # Print None
